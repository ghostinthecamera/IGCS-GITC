...
namespace IGCS
{
        ...
        const DirectX::XMVECTOR& playerRotation) const noexcept
    {
        ...
        // Transform the local offset to world space using the player's rotation quaternion
        const XMVECTOR worldOffset = XMVector3Rotate(localOffset, playerRotation);
    ...
    void Camera::captureCurrentRelativeOffset() noexcept
    {
       ...
       // Camera position is loaded directly from class member _toolsCoordinate below
        ...
        const XMVECTOR cameraRotVec = Utils::generateEulerQuaternion(cameraRotation, MULTIPLICATION_ORDER, false, false, false);
    ...
    void Camera::interpolateRotation(float lerpTime) noexcept
    {
        ...
        setRotation(Utils::QuaternionToEulerAngles(newRotQ, MULTIPLICATION_ORDER));
        ...
        const XMVECTOR& playerRotation) noexcept
    {
        ...
        setTargetPitch(NEGATE_PITCH ? -lookAtRotation.x : lookAtRotation.x);
        setTargetYaw(NEGATE_YAW ? -lookAtRotation.y : lookAtRotation.y);
        setTargetRoll(lookAtRotation.z); // We dont need to apply negation as the roll is calculated independently and is already in the tool coordinate system.
    }
    ...
    DirectX::XMMATRIX Camera::getViewMatrix() const
    {
        ...
        // Build view matrix - transpose of world rotation
        ...
        XMVECTOR negTranslation = XMVector3Transform(XMVectorNegate(posVec), viewRotation);

        // Assemble final view matrix
        XMMATRIX viewMatrix = viewRotation;
    ...
    DirectX::XMMATRIX Camera::getProjectionMatrix() const
    {
        ...
        // Calculate aspect ratio (assume 16:9 if we can't get actual viewport)
        float aspectRatio = 16.0f / 9.0f; // Default fallback

        return XMMatrixPerspectiveFovLH(fov, aspectRatio, nearZ * 2.0f, 10000.0f);
    }
} // namespace IGCS
