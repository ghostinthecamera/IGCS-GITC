...
namespace IGCS
{
    ...
    public:
        ...
        // It may seem counterintuitive, but if we want this from the player perspective, we need to do the inverse
        // of the constants defined in GameConstant.h (kForwardSign and kRightSign) to keep this in the games coordinate system
        // rather than the tools. Up is not needed as up is the same direction whether in the tools coordinate system or the games.
        void addLookAtTargetForward(float amount) noexcept { _lookAtTargetOffset.z += -kForwardSign * amount; }     // Forward/backward relative to player
        void addLookAtTargetRight(float amount) noexcept { _lookAtTargetOffset.x += -kRightSign * amount; }         // Right/left relative to player  
    ...
    private:
        ...
        void applyFixedMountTransformation(const XMVECTOR& playerPosVec, const XMVECTOR& playerRotVec
        ) noexcept;
		...
		#ifdef DEBUG
			EulerOrder _eulerOrder = Utils::EulerOrder::XYZ;
		#else
			EulerOrder _eulerOrder = MULTIPLICATION_ORDER;
		#endif
