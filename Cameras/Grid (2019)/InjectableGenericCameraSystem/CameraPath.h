...
namespace IGCS
{
	...
	public:
		...
		// Path look-at functionality
		[[nodiscard]] DirectX::XMFLOAT3 calculatePathLookAtRotation(const DirectX::XMFLOAT3& cameraPos,
			const DirectX::XMFLOAT3& targetPos) const noexcept;
		[[nodiscard]] DirectX::XMFLOAT3 calculatePathOffsetTargetPosition(const DirectX::XMFLOAT3& playerPos,
			const DirectX::XMVECTOR& playerRotation) const noexcept;
		void applyPathLookAt(float deltaTime) noexcept;

		// Path speed matching functionality
		void updateVehicleSpeedTracking(float deltaTime) noexcept;
		[[nodiscard]] float calculateVehicleSpeed(const DirectX::XMVECTOR& currentPos,
			const DirectX::XMVECTOR& previousPos, float deltaTime) const noexcept;
		[[nodiscard]] float mapVehicleSpeedToPathMultiplier(float vehicleSpeed) const noexcept;
		void resetSpeedMatching() noexcept;
		void updatePathLookAtTarget() noexcept;

		// Get the current path look-at target position for visualization
		[[nodiscard]] DirectX::XMFLOAT3 getCurrentPathLookAtTargetPosition() const noexcept { return _currentPathLookAtTarget; }
		[[nodiscard]] bool hasValidPathLookAtTarget() const noexcept { return _hasValidPathLookAtTarget; }
		[[nodiscard]] bool getPathLookAtEnabled() const noexcept { return _pathLookAtEnabled; }
		[[nodiscard]] static int getSampleSize() { return _samplesize; }
	...
	private:
		...
		// Path Lookat
		bool _pathLookAtEnabled = false;
		DirectX::XMFLOAT3 _pathLookAtOffset = { 0.0f, 0.0f, 0.0f };
		DirectX::XMVECTOR _currentPathLookAtRotation = XMQuaternionIdentity();
		bool _pathLookAtInitialized = false;
		float _pathLookAtSmoothness = 5.0f;


		// Path speed matching settings cache
		bool _pathSpeedMatchingEnabled = false;
		float _pathSpeedScale = 1.0f;
		float _pathSpeedSmoothness = 3.0f;
		float _pathMinSpeed = 0.1f;
		float _pathMaxSpeed = 3.0f;
		float _pathBaselineSpeed = 50.0f;

		// Vehicle speed tracking state
		DirectX::XMVECTOR _previousVehiclePosition = XMVectorZero();
		float _currentVehicleSpeed = 0.0f;
		float _targetPathSpeedMultiplier = 1.0f;
		float _currentPathSpeedMultiplier = 1.0f;
		bool _vehicleSpeedInitialized = false;

		// Store the current path look-at target position for visualization
		DirectX::XMFLOAT3 _currentPathLookAtTarget{ 0.0f, 0.0f, 0.0f };
		bool _hasValidPathLookAtTarget{ false };
	};
}