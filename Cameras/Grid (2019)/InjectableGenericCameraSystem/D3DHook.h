...
namespace IGCS {
    ...
    public:
        ...
        // Look-at target visualization methods
        void createLookAtTargetSphere(float radius = 0.5f, const DirectX::XMFLOAT4& color = DirectX::XMFLOAT4(0.0f, 1.0f, 0.0f, 0.8f));
        void renderLookAtTarget(const DirectX::XMMATRIX& viewMatrix, const DirectX::XMMATRIX& projMatrix) const;
        void renderFreeCameraLookAtTarget();
        void renderPathLookAtTarget(const DirectX::XMMATRIX& viewMatrix, const DirectX::XMMATRIX& projMatrix) const;
    ...
    private:
        ...
        void createInterpolatedDirections(const std::vector<InterpSample>& interpSamples,
                                          std::vector<SimpleVertex>& interpolatedDirectionVertices, UINT nodeStartIndex, UINT nodeCount) const;
        ...
        ID3D11Buffer* _lookAtTargetVertexBuffer = nullptr;
        ID3D11Buffer* _lookAtTargetIndexBuffer = nullptr;
        UINT _lookAtTargetIndexCount = 0;
        float _lookAtTargetSize = 0.5f; // Default sphere size
