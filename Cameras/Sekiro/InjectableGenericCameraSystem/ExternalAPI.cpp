...
{
	IGCS::MessageHandler::logDebug("Starting IGCS Connecter session");
	System* activesystem = Globals::instance().getCurrentSystem();

	// start the session
	return activesystem->startIGCSsession(type);
}
...
{
	System* activesystem = Globals::instance().getCurrentSystem();
	activesystem->stepCameraforIGCSSession(stepAngle);
}
...
{
	System* activesystem = Globals::instance().getCurrentSystem();
	activesystem->stepCameraforIGCSSession(stepLeftRight, stepUpDown, fovDegrees, fromStartPosition);
}
