...
public:
	...
	inline static constexpr std::array<IdToXInput, 14> kIdToXInputTable{ {
			...
			/* 4  */ {  4, XINPUT_GAMEPAD_Y,              Gamepad::button_t::Y      },
			/* 5  */ {  5, XINPUT_GAMEPAD_DPAD_UP,        Gamepad::button_t::UP     },
			/* 6  */ {  6, XINPUT_GAMEPAD_DPAD_DOWN,      Gamepad::button_t::DOWN   },
			/* 7  */ {  7, XINPUT_GAMEPAD_DPAD_LEFT,      Gamepad::button_t::LEFT   },
			/* 8  */ {  8, XINPUT_GAMEPAD_DPAD_RIGHT,     Gamepad::button_t::RIGHT  },
			/* 9  */ {  9, XINPUT_GAMEPAD_LEFT_SHOULDER,  Gamepad::button_t::LB     },
			/* 10 */ { 10, XINPUT_GAMEPAD_RIGHT_SHOULDER, Gamepad::button_t::RB     },
	...
	/* --------------------------------------------------------------------------
	   constexpr helper – translate old ID → XInput mask (or std::nullopt)
	   -------------------------------------------------------------------------- */
	[[nodiscard]]
	static constexpr optional<Gamepad::button_t> idToXInputMask(int id) noexcept
	{
		for (auto const& entry : kIdToXInputTable)
			if (entry.id == id)
				return entry.button;
		return std::nullopt;        // unknown number
	}
