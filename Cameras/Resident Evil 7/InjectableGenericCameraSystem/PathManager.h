...
namespace IGCS
{
    ...
    public:
		...
		void setPathManagerState(PathManagerState state) { _pathManagerState = state; }
		[[nodiscard]] PathManagerState getPathManagerState() const { return _pathManagerState; }
		[[nodiscard]] PathManagerStatus getPathManagerFunction() const { return _pathManagerStatus; }

        PathManagerState _pathManagerState = Disabled;
        PathManagerStatus _pathManagerStatus = Idle;
        PathManagerStatus _preScrubbingStatus = Idle;
        ...
        bool reshadeAddonisConnected() const;
        void addReshadePath();
        void appendStateToReshadePath(const std::string& pathName);
        void appendStateAfterReshadeSnapshot(const std::string& pathName, int snapshotIndex);
    ...
    private:
        ...
        struct ActionLock {
            ...
            ActionLock(std::atomic<bool>& f) : flag(f), acquired(false) {
                bool expected = false;
                acquired = flag.compare_exchange_strong(expected, true);
            }
            ~ActionLock() { if (acquired) flag = false; }
