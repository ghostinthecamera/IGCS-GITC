...
namespace IGCS::PathUtils
{
	...
	XMVECTOR simulateHandheldCamera(float deltaTime, float driftIntensity, float jitterIntensity,
		...
		const float springConstant = 2.0f;  // Reduced from 5.0f to allow more drift
		const float dampingFactor = 1.2f;   // Reduced from 2.0f to allow more motion
		...
		float jitterX = perlinNoise1D(time * 8.0f) * jitterIntensity;
		float jitterY = perlinNoise1D(time * 8.0f + 500.0f) * jitterIntensity;
		float jitterZ = perlinNoise1D(time * 8.0f + 1000.0f) * jitterIntensity * 0.5f;
